...
1. `[blocked]` 台股重大新聞自動抓取
   ...
   - Blocker：
     - 本 repo 內沒有找到 Hermes agent cron job 程式、設定或外部路徑，無法確認 Hermes 的實際抓取來源與排程，只能透過共用 `posts` 表和去重策略整合。
     - Production 需要設定 `CRON_SECRET`，否則 Cron endpoint 會拒絕寫入，避免公開端點被濫用。
