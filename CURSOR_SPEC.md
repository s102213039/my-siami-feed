...
1. `[blocked]` 台股重大新聞自動抓取
   ...
   - Hermes 整合方式：
     ...
     - 已將該行為移植到本專案新聞 Cron：保留 Google News `finance` RSS Top 10，寫入網站 `posts`，並在有 `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` 時同步發 Telegram。
     - 已新增相容端點 `/api/cron/fetch-news`，也可用 `Authorization: Bearer <CRON_SECRET>` 手動觸發同一套新聞整合流程。
   - Blocker：
     - PM 已在 Vercel 設定 `CRON_SECRET`。
     - 若要 production 也發 Telegram，仍需在 Vercel 設定 `TELEGRAM_BOT_TOKEN` 與 `TELEGRAM_CHAT_ID`；未設定時只會寫入網站 feed，不會發 Telegram。
...
2. `[todo]` 即時股價展示
   ...
   - 初始標的：
     - `2330.TW` 台積電
     - `2603.TW` 長榮
     - 萬海（需確認代號，可能是 `2615.TW`）
     - `00403A`（需確認 Yahoo Finance / 資料源代號）
     - `0050.TW` 元大台灣50
...
- 依 PM 修正，改為移植 Hermes 既有 cron job 行為本身：
  ...
  - 新增可選 Telegram 通知：production 設定 `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` 後，Cron 會同步發送「今日金融/股市新聞摘要 (Top 10)」。
  - 已驗證 `npm run lint`、`npm run build`、本機 `/api/cron/fetch-news?dryRun=1` 回傳 `hermesLegacyCandidates: 10`。
  - 目前 production Telegram 通知仍等待 PM 在 Vercel 補上 Telegram 環境變數。
