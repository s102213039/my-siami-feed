...
3. `[todo]` **驗證 production 新聞 Cron 與資料**（延續自 5/29，今日優先）
...
**昨日（5/29）結論**：以規格書對齊與排程建議為主，尚未消化五項待辦中的實作或驗證。今日應從「可觀測、可關閉」的項目開始。

優先順序建議：

1. `[doing]` **驗證 production 新聞 Cron 與資料**（最高優先）
   - 08:55 後檢查 Vercel Cron logs；必要時以 `dryRun=1` 或 Bearer token 手動觸發比對。
   - 在 Supabase 確認 `posts.detail` 欄位與今日貼文內容格式。
2. `[todo]` **手機版歷史側欄** — 實作 drawer / tab，完成後跑 RWD 實機檢查。
3. `[todo]` **新聞分類自動化** — 先在 `taiwanStockNews.ts` 做關鍵字映射，不新增付費 API。
4. `[todo]` **AI 回覆上下文** — 擴充 `commentReply.ts`；`NEWS_DIGEST_USE_AI` 需 PM 明示才開。
5. `[todo]` **測試策略** — 先補 `taipei.ts` 與去重邏輯單元測試，再考慮 e2e。

**本日文件**：將工作區落後的 `CURSOR_SPEC.md`（仍顯示 5/21 / 40%）同步至 5/30 版本，並把 5/29 待辦正式列入第 4 節項目 3～7。
