...
- `supabase/migrations/20260523120000_add_posts_detail.sql`：posts.detail 欄位（主畫面「文章內容」簡短總結）
...
- [x] 新聞簡短總結寫入 `posts.detail`（約 360 字內＋引導點來源）；完整內文由 `source_url` 外連
- [x] 新寫入文章需有可用的 RSS 摘要素材才入庫（`buildFallbackPost` / Gemini 路徑）
...
3. `[done]` 新聞閱讀體驗與歷史瀏覽（2026-05-24～05-27）
   - 主畫面預設台北時區當日新聞；較早新聞由 `NewsArchiveSidebar` 依日期瀏覽。
   - 卡片主畫面直接顯示「文章內容」（`detail`）；留言區維持收合。
   - `posts.detail` 自 5/27（`9489ff8`）起改為**簡短總結**（非 RSS 全文）；文末提示「完整內容請點擊來源閱讀」。
   - AI 留言 prompt 已帶入 `detail`；`main` 已含 5/24 Cron 修正與 5/27 總結邏輯。
...
### 昨日建議推進（2026-05-27）— 執行結果

> 5/27 有功能程式碼提交（`9489ff8` 已合併 `main`）；規格書同步於本日（5/28）一併補登。下列五項仍多為 `[todo]`／`[doing]`。

1. `[doing]` **驗證 production 新聞 Cron 與 migration** — 未標記完成
   - `main` 已部署含 `detail` 與 Cron 修正；仍需 PM／Agent 對照 Supabase 與 Vercel Logs。
2. `[todo]` **手機版與側欄 RWD** — 未開始
3. `[todo]` **新聞分類自動化** — 未開始
4. `[todo]` **AI 回覆與舊文補強** — 未開始（`detail` 語意已改，prompt 調整可一併處理）
5. `[todo]` **最小測試骨架** — 未開始

**5/27 額外完成（不在原五項內）：**

- `[done]` **`detail` 改為簡短總結**（`9489ff8`）：`lib/news/taiwanStockNews.ts` 不再保存 RSS 全文；主畫面顯示精簡說明，完整報導靠來源連結。

### 今日建議推進（2026-05-28）

> 由每日 Cron 自動化（01:00 UTC）觸發更新。`main` 已為 `9489ff8`；今日 08:55 排程將寫入**新格式** `detail`，驗證應涵蓋 5/26、5/27 兩輪 Logs。

1. `[doing]` **驗證 production 新聞 Cron、migration 與新 `detail` 格式**（最高優先）
   - Supabase：確認 `posts.detail` 欄位存在；抽查 5/28 08:55 後新文是否為簡短總結（非過長原文）。
   - Vercel Logs：對照 **2026-05-26、2026-05-27** 台灣時間 08:55 是否 200、是否有寫入 `posts`。
   - 手動：`GET /api/cron/taiwan-stock-news?dryRun=1`（本機）；production 需 `x-vercel-cron: 1` 或 `Authorization: Bearer <CRON_SECRET>`。
   - 線上：https://my-siami-feed.vercel.app/ 今日新聞卡片「文章內容」是否可讀、來源連結是否正常。

2. `[todo]` **手機版與側欄 RWD**
   - `NewsArchiveSidebar` 小螢幕改抽屜或頂部日期選擇器。
   - `detail` 已縮短，但仍需驗證股價列、搜尋列、側欄不橫向溢出。

3. `[todo]` **新聞分類自動化**
   - 在 `lib/news/taiwanStockNews.ts` 依關鍵字／來源對應 `categories`（半導體、航運、總經等）。

4. `[todo]` **AI 回覆與舊文補強**
   - 5/24～5/26 若曾寫入長篇 `detail`：可選一次性腳本截斷或 UI 標示「舊格式／僅摘要」。
   - `lib/ai/commentReply.ts`：prompt 明確「`detail` 為編輯總結、非全文」，並要求引用 `source_url`。

5. `[todo]` **最小測試骨架**
   - 新增 `npm run test`，覆蓋 `lib/dates/taipei.ts`、`buildFallbackPost` 的 `detail` 長度與去重邏輯。
...
- 每日 Cron 自動化（01:00 UTC）觸發規格書規劃（commit `360e6fc`，分支 `cursor/-bc-e0c2d916-...`）：對齊 5/26 內容、建立「今日建議推進（2026-05-27）」；當時 `main` 尚未含該文件更新。
- **功能程式碼**（`9489ff8`，已合併 `main`）：
  - `lib/news/taiwanStockNews.ts`：`posts.detail` 改為每則新聞的**簡短總結**（約 360 字＋引導點來源），不再保存 RSS 全文。
  - `content` 與 `detail` 對齊同一份說明文字，供列表、主畫面與 AI prompt 使用。
  - 完整報導交由 `source_url` 外連；降低主畫面資訊過載與手機版溢出風險。
- 5/26～5/27 五項「建議推進」多數仍為 `[todo]`／`[doing]`；production Cron／migration 驗證待 PM 或 Agent 在 Vercel／Supabase 確認。

### 2026-05-28

- 每日 Cron 自動化（01:00 UTC）觸發本說明與規格書同步。
- 將 `CURSOR_SPEC.md` 自 5/21 版補齊至含 5/24～5/27 功能摘要；進度約 **62%**。
- 新增「昨日建議推進（2026-05-27）執行結果」與「今日建議推進（2026-05-28）」；驗證項納入新 `detail` 格式與 5/28 08:55 排程觀察。
- 今日開發焦點（見第 4 節）：**production 驗證（含新 detail）** → 手機版 RWD → 分類自動化 → AI／舊文 → 測試骨架。
