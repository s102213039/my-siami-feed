...
**昨日回顧（6/15）**：僅文件同步，無程式碼 commit。延續 6/14 盤點，將規格書更新至 2026-06-15、新增任務 E（歷史 archive 搜尋/分類），並建議 A → B → C 的當日組合節奏。6/15 建議的「手機版歷史側欄」仍未動工。

**累積待辦狀態**：自 5/27 最後一次功能 commit（`9489ff8`）以來，已連續 20 天無新功能合併；下一階段五項 `[todo]` 均維持未開始。手機版歷史側欄已連續三日列為頭號優先項。

**今日建議聚焦（擇一或依序推進）**：

| 順序 | 任務 | 預估影響 | 具體步驟 |
|------|------|----------|----------|
| A | 手機版歷史側欄 | 高（使用者體驗） | 在 `NewsArchiveSidebar` 加 `< lg` 可收合按鈕或底部 sheet；`app/page.tsx` 調整主欄與側欄堆疊順序；手機寬度下主內容優先、側欄預設收合 |
...
**建議今日第一個實作任務**：A（手機版歷史側欄）— 連續三日列為頭號優先項，風險低、可見度高，完成後可直接勾選「手機版與資訊密度優化」部分子項。

**若今日有實作時間，建議組合**：優先完成 A（預估 2–3 小時）→ 接著執行 B（dryRun 驗證，約 15 分鐘）→ 若仍有餘力，啟動 C 的關鍵字對照表雛形（可先涵蓋半導體、航運、總經三類）。

**阻塞項提醒**：
- 新聞分類若要用 Gemini，需 PM 確認 `NEWS_DIGEST_USE_AI=true` 與 API 用量。
- Production Cron log 需 Vercel dashboard 權限；Agent 可用 dryRun 驗證端點邏輯。
- 本工作分支 `CURSOR_SPEC.md` 曾落後於其他自動化分支（6/13–6/15 文件 commit 在獨立分支）；本次已合併至 2026-06-16 版。
