...
- Git 無新 commit；無程式變更記錄。
- 建議 PM / Agent 可手動確認：production 是否已套用 `posts.detail` migration、05-24 後 Cron 是否成功寫入今日 feed。

### 2026-05-29（今日建議）

優先順序建議：

1. `[todo]` **驗證 production 新聞 Cron 與資料**
   - 檢查 Vercel Cron 執行紀錄與 `/api/cron/taiwan-stock-news` 最近一次成功時間。
   - 確認 Supabase `posts.detail` 欄位存在；若 production 尚未 migration，需依第 3 節「先確認」流程執行。
2. `[todo]` **手機版與歷史側欄 UX**
   - 小螢幕下歷史列表改為可收合 drawer 或頂部 tab，避免與主 feed 垂直堆疊過長。
3. `[todo]` **新聞分類自動化**
   - 依 RSS 來源或關鍵字映射到 `categories`（半導體、航運、總經等），減少全部落在 `finance`。
4. `[todo]` **AI 回覆品質**
   - 留言回覆時帶入更多上下文（`detail` + `source_url` + 同日相關新聞標題）；評估是否由 PM 開啟 `NEWS_DIGEST_USE_AI=true`（有 API 用量）。
5. `[todo]` **測試策略**
