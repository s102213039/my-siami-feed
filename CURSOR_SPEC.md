...
1. `[blocked]` 台股重大新聞自動抓取
   ...
   - Hermes 整合方式：
     - 已新增相容端點 `/api/cron/fetch-news`，Hermes agent 可用 `Authorization: Bearer <CRON_SECRET>` 呼叫同一套新聞整合流程。
     - Hermes 若已有獨立 Python 腳本，建議只保留抓取來源或外部排程角色，寫入統一交給本專案 API，避免兩邊重複寫入。
   - Blocker：
     - PM 已在 Vercel 設定 `CRON_SECRET`；本次會透過 commit / push 重新部署，讓 production Cron 吃到最新環境變數。
     - 本 repo 內仍沒有找到 Hermes agent 實際 cron job 程式或 `scripts/stock_news_scraper.py`，目前只能提供相容 API 與共用 `posts` 去重整合；若要改 Hermes 腳本本身，需 PM 提供實際檔案或 repo 位置。
