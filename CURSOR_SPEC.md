...
- 當前進度：約 65%（核心 feed、留言 AI、新聞 Cron、股價 ticker、今日/歷史新聞瀏覽已完成；待 RWD 精修、新聞分類、AI 上下文與測試）
- 專案角色分工：
  - PM / 需求提出：Yanli
  - 開發執行：Cursor Agent
...
- 更新本規格書：進度約 65%，第 4 節項目 3 改為 `[doing]`，新增項目 7–8 並標 `[done]`。
- **今日建議優先順序**（接續推進）：
  1. `[doing]` 完成項目 3 剩餘 RWD：股價列與歷史側欄手機版。
  2. `[todo]` 項目 4 新聞分類自動化（低風險、可獨立 PR）。
  3. `[todo]` 項目 6 最小 smoke test，鎖定 Cron 與行情 API。
  4. `[todo]` 項目 5 AI 上下文（需設計 prompt，避免 token 暴增）。
  5. 合併並 push 功能分支後，確認 Vercel preview / production 今日 feed 與 08:55 Cron 寫入正常。
