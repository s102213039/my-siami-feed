...
6. `[todo]` 手機版歷史新聞 drawer（**今日最高優先**）
   - 現象：`NewsArchiveSidebar` 在手機上全寬堆疊於主 feed 上方，佔用大量垂直空間。
   - 目標：`< lg`  breakpoint 改為浮動按鈕 + drawer/sheet；`lg+` 維持現有左側 sticky。
...
- 確認下一階段最高優先：**手機版 `NewsArchiveSidebar` drawer**（已 pending 約 4 天）。
